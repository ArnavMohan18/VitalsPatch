...
// =====================================================================
//  CHANGE 1 — sensors.h
//  Add this declaration alongside the existing initSensors / readSensors:
// =====================================================================

void tickSensors();   // call every loop() iteration for streaming HR


// =====================================================================
//  CHANGE 2 — vitals_sensor_module.ino  loop()
//  Add tickSensors() as the very first statement so it runs on every
//  iteration regardless of BLE state or the 1-second timer.
// =====================================================================

void loop() {

  tickSensors();   // <-- ADD THIS LINE (drains FIFO, updates hrBeatAvg)
  ...
  uint32_t lastMotion = 0;   // NOTE: this was already in the original code

  if (connected && millis() - lastSample >= 1000) {
    lastSample = millis();

    SensorPacket data;
    if (readSensors(data)) {   // now non-blocking — returns immediately
      ...
      if (sampleIndex >= RAW_SAMPLE_COUNT) {
        for (int i = 0; i < SAMPLE_COUNT; i++) {
          ...
          for (int j = 0; j < 3; j++) {
            ...
            if (value >= 0) { sum += value; validCount++; }
          }
          hrBuf[i] = (validCount > 0) ? sum / validCount : -1;
        }
        sendPacket();
        sampleIndex = 0;
      }
